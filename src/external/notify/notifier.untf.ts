...
export class UntfNotifier extends Notifier {
  ...
  constructor(@Inject(ENV) private readonly env: Env) {
    ...
    this.topic = this.env.untf.topic;
  }
  ...
  notify(message: string): void {
    this._notify(message).catch((err) => {
      log.error('Notification failure', { stack_trace: stacktrace(err) });
    });
  }

  private async _notify(message: string): Promise<void> {
    const url = `${this.endpoint}/api/send/v1`;
    const body: UntfSendRequest = { topic: this.topic, message };
